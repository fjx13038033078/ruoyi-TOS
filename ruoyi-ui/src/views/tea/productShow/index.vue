<template>
  <div>
    <!-- 商品管理页面内容 -->
    <div v-if="activeTab === 'productShow'">
      <div class="app-container">
        ...
        <el-row :gutter="20" class="product-card-container">
          <el-col :span="6" v-for="product in productList" :key="product.productId" class="product-card">
...
export default {
  data() {
    return {
      // 遮罩层
      loading: true,
      activeTab: 'productShow', // 当前激活的选项卡，默认为商品管理
      productList: [], // 商品列表数据
      shopOptions: [], //店铺选项列表
      // 总条数
      totalProducts: 0,
  ...
  methods: {
    ...
    fetchProducts() {
      ...
      listProducts(this.queryParams).then(response => {
        ...
        // 遍历 productList，获取每个商品的 productImage
        const productImagePromises = this.productList.map(product =>
          getProduct(product.productId).then(productDetails => {
            product.productImage = productDetails.data.productImage;
        ...
        Promise.all(productImagePromises).then(() => {
          this.loading = false;
        });
      });
    },
